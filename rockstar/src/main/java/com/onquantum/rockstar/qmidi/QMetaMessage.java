...
public class QMetaMessage extends QMidiMessage {
    ...
    public void setMessage(int type, byte[] data, int length)
        ...
        if ((length > 0 && length > data.length) || length < 0) {
            throw new QInvalidMidiDataException("Length out of the bound " + length);
        }
    ...
    public byte[] getData() {
        ...
        System.arraycopy(data, (length - dataLength), array, 0, dataLength);
        return array;
    }
    ...
    public Object clone() {
        ...
        QMetaMessage event = new QMetaMessage(newData);
        return event;
    }
